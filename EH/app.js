...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function buildNotationBox(question) {
    return `
      <div class="note-box">
        <b>📝 Notation & Units:</b>
        Use standard hydrology notation consistently. Unless stated otherwise, take discharge in $\\text{m}^3/\\text{s}$, rainfall/runoff depth in $\\text{cm}$ or $\\text{mm}$, catchment area in $\\text{km}^2$, and time in hours.
      </div>
    `;
  }
  ...
  function buildApproachBox(question) {
    return `
      <div class="note-box">
        💡 <b>Approach / Hint:</b> Start with the core formula or definition, show unit conversions clearly, solve in logical steps, and box the final answer. For theory questions, write crisp headings first and then explain point-wise in textbook style.
      </div>
    `;
  }
  ...
  function openModal(question) {
    ...
    modalQuestionText.innerHTML = question.question;
  ...
  function renderMath(root = document.body, attempt = 0) {
    if (!root) return;
